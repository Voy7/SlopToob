...
export default function isNicknameValid(name: string): true | string {
  if (name.length < Settings.nicknameMinLength) return `Nickname must be at least ${Settings.nicknameMinLength} characters.`
  if (name.length > Settings.nicknameMaxLength) return `Nickname must be at most ${Settings.nicknameMaxLength} characters.`
