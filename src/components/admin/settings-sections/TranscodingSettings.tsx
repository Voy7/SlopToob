  ...
  return (
    <LoadingBoundary>
      <SettingGroup>
        ...
        <SelectDropdown
          ...
          {videoQualities.map((quality, index) => (
            <SelectItem
              ...
              label={quality.name}
              onClick={() => maxVideoQuality.setValue(index)}
              className="py-1.5"
            />
          ))}
        </SelectDropdown>
        ...
        <SelectDropdown
          ...
          {audioQualities.map((quality, index) => (
            <SelectItem
              ...
              label={quality.name}
              onClick={() => maxAudioQuality.setValue(index)}
              className="py-1.5"
            />
          ))}
        </SelectDropdown>
        ...
        <div className="h-4" />
        <ToggleOption label="Enable Volume Normalization" {...enableVolumeNormalization} />
        <NumberOption label="Target Integrated" type="float" {...loudnormTargetIntegrated} />
        <NumberOption label="Target Peak" type="float" {...loudnormTargetPeak} />
        <NumberOption label="Loudness Range" type="integer" {...loudnormRange} />
      </SettingGroup>
    </LoadingBoundary>
  )
}
